...
function renderWeeklyGrid() {
    ...
    if (horarios.length === 0) {
        gridContainer.innerHTML = '<p style="grid-column: 1/-1; padding: 20px;">Selecione uma turma válida.</p>';
        return;
    }
...
function handleSlotClick(dia, horario) {
    ...
    if (tipo === 'intensiva') return alert("Para intensivas, configure as datas e clique no botão 'Adicionar à Grade'.");
    ...
    store.addAllocation({
        ...
        disciplina, docente, tipo,
        diaSemana: dia, horario,
...
function renderMonthlyCalendar() {
    ...
    if (store.rawData && store.rawData.turmas) {
        const t = store.rawData.turmas.find(x => x.turma_id === store.selectedTurma);
...
function generateCalendarGrid(container, turmaId, docenteName, start, end, titleHTML) {
    ...
    if (conflictCount > 0) {
        ...
        alertDiv.style.cssText = "background: #c0392b; color: white; padding: 10px; text-align: center; margin-bottom: 20px; border-radius: 4px; font-weight: bold;";
    ...
    if (turmaId) {
        ...
        const m = store.rawData.horarios['manha_extendido'] || [];
        const t = store.rawData.horarios['tarde_extendido'] || [];
        slotsToRender = [...m, ...t];
    }
                    ...
                    slotsToRender.forEach((slotTime, index) => {
                        ...
                        if (isIntervalo) {
                            ...
                            if (isConflict) {
                                ...
                                content = `⚠️ CHOQUE! <br>` + eventsInSlot.map(e => {
...
function checkTeacherConflict(docente, dia, horario) {
    return store.allocations.find(a => 
        a.docente === docente && 
        a.diaSemana == dia && 
        a.horario === horario
    );
